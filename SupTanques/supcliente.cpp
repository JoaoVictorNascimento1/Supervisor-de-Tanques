...
void SupCliente::main_thread(void)
{
  ...
  while (!encerrarCliente && isConnected())
  {
    ...
    try
    {
      ...
      if (sock.read_uint16(cmd) != mysocket_status::SOCK_OK) throw 402;

      // Se resposta nao for CMD_OK, throw 403
      if (cmd != CMD_OK) throw 403;
      // Leh os dados (com timeout)
      // Em caso de erro, throw 404
      int16_t cmd;
      mysocket_status result = sock.read_int16(cmd); // Timeout de 10 segundos

      if (result != mysocket_status::SOCK_OK) {
      throw 404; // Lança a exceção em caso de erro
}
